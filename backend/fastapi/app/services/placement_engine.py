...
try:
    ...
    detect_bays_xy = detect_levels = None
...
def detect_active_grid(existing_members: list) -> dict:
    ...
    Prefers bay-line detection over raw member bbox (avoids off-grid junk).
    """
    if not existing_members:
        return _default_grid()

    if detect_bays_xy:
        xs, ys = detect_bays_xy(existing_members)
        if xs and ys and len(xs) >= 2 and len(ys) >= 2:
            z_levels = detect_levels(existing_members) if detect_levels else [0]
            min_z = min(z_levels) if z_levels else 0
            max_z = max(z_levels) if z_levels else 0
            return {
                "origin":     {"x": min(xs), "y": min(ys), "z": min_z},
                "extent":     {"x": max(xs), "y": max(ys), "z": max_z},
                "width_x":    max(xs) - min(xs),
                "width_y":    max(ys) - min(ys),
                "height":     max_z - min_z,
                "spacing_x":  _infer_spacing(xs),
                "spacing_y":  _infer_spacing(ys),
                "story_h":    _infer_spacing([z for z in z_levels if z > 0]) or 3500,
                "has_model":  True,
                "from_bays":  True,
            }
...
def align_footprint_to_grid(
    ...
    """
    Final placement correction — keeps structure INSIDE the Tekla grid.
    Towers are generated around local (0,0); this shifts so min XY >= grid origin.
    """
    if not elements or strategy == "append":
        return elements

    min_x, max_x, min_y, max_y, _, _ = _element_bbox(elements)
    gx = grid_ctx["origin"]["x"] if grid_ctx.get("has_model") else 0.0
    gy = grid_ctx["origin"]["y"] if grid_ctx.get("has_model") else 0.0

    if strategy == "center" and grid_ctx.get("width_x", 0) > 500:
        gcx = (grid_ctx["origin"]["x"] + grid_ctx["extent"]["x"]) / 2
        gcy = (grid_ctx["origin"]["y"] + grid_ctx["extent"]["y"]) / 2
        scx = (min_x + max_x) / 2
        scy = (min_y + max_y) / 2
        dx, dy = gcx - scx, gcy - scy
    else:
        # Corner anchor: structure bbox min corner → grid origin (A-1)
        dx, dy = gx - min_x, gy - min_y

    if abs(dx) < 0.5 and abs(dy) < 0.5:
        return elements
    return apply_offset(elements, {"x": dx, "y": dy, "z": 0.0}, 0.0)
...
def compute_placement_origin(
    ...
    if strategy == "inline" and grid_ctx["has_model"]:
        oz = grid_ctx["origin"].get("z", 0)
        return {
            "x": grid_ctx["origin"]["x"],
            "y": grid_ctx["origin"]["y"],
            "z": oz if oz >= 0 else 0.0,
        }

    if strategy == "center" and grid_ctx["has_model"]:
        cx = (grid_ctx["origin"]["x"] + grid_ctx["extent"]["x"]) / 2
        cy = (grid_ctx["origin"]["y"] + grid_ctx["extent"]["y"]) / 2
        return {"x": cx, "y": cy, "z": 0.0}

    # Towers on existing site — align step handles footprint; start at grid origin
    if "tower" in stype and grid_ctx["has_model"]:
        return {
            "x": grid_ctx["origin"]["x"],
            "y": grid_ctx["origin"]["y"],
            "z": 0.0,
    ...
    # "auto" / "append" — only when explicitly requested
...
# ── STRUCTURE FOOTPRINT & BRACE VALIDATION ───────────────────────────────────

def compute_structure_footprint(params: dict) -> dict:
    """Local-coordinate bounding box for the generated structure."""
    stype = str(params.get("structure_type", "building")).lower()

    if stype == "bridge":
        span = float(params.get("span", 120000))
        width = float(params.get("width", 12000))
        deck_z = float(params.get("deck_height", 8000))
        return {
            "min_x": 0, "max_x": span,
            "min_y": 0, "max_y": width,
            "min_z": 0, "max_z": deck_z,
        }

    if stype == "tower_transmission":
        hw = float(params.get("base_width", 8000)) / 2
        arms = params.get("arm_levels") or [
            {"length": 9000}, {"length": 8000}, {"length": 7000},
        ]
        arm_reach = max(float(a.get("length", 0)) for a in arms) if arms else hw
        hw = max(hw, arm_reach, arm_reach * 0.6)
        h = float(params.get("height", 45000))
        return {"min_x": -hw, "max_x": hw, "min_y": -hw, "max_y": hw, "min_z": 0, "max_z": h}

    if "tower" in stype or stype == "tower_hexagonal":
        hw = float(params.get("base_width", 4000)) / 2
        h  = float(params.get("height", 30000))
        mast = float(params.get("mast_height", 2500))
        arm = max(hw * 0.55, float(params.get("top_width", hw)) * 0.35)
        ladder_w = min(400, hw * 0.15)
        return {
            "min_x": -hw - ladder_w - arm,
            "max_x": hw + arm,
            "min_y": -hw - arm,
            "max_y": hw + arm,
            "min_z": 0,
    ...
    building_like = (
        "building", "warehouse", "pipe_rack", "shed", "portal_frame",
        "office_building", "residential", "stadium", "substation", "parking",
        "industrial_frame",
    )
    bays_x = int(params.get("bays_x", 4))
    bays_y = int(params.get("bays_y", 3))
    sx     = float(params.get("spacing_x", 6000))
    sy     = float(params.get("spacing_y", 6000))
    sh     = float(params.get("story_height", 3500))
    stories = int(params.get("stories", 2))
    total_h = sh * stories
    if params.get("roof_type") == "gable":
        total_h += min(sy * bays_y * 0.25, 2500)
    if stype in building_like or stype.endswith("_building"):
        return {
            "min_x": 0, "max_x": bays_x * sx,
            "min_y": 0, "max_y": bays_y * sy,
            "min_z": 0, "max_z": total_h,
        }

    # Fallback — use span/width if present (long structures)
    span = float(params.get("span", bays_x * sx))
    width = float(params.get("width", bays_y * sy))
    return {
        "min_x": 0, "max_x": max(span, bays_x * sx),
        "min_y": 0, "max_y": max(width, bays_y * sy),
        "min_z": 0, "max_z": total_h,
    }


def validate_structure_geometry(elements: list, params: dict) -> tuple:
    """
    Pre-placement validation:
      - Reject members outside structure footprint
      - Reject cross-face braces on rectilinear buildings (brace must stay on one face)
      - Snap brace endpoints to nearest grid line on their face
    """
    footprint = compute_structure_footprint(params)
    tol       = 250.0
    stype     = str(params.get("structure_type", "building")).lower()
    ...
    # Known grid lines for snapping (buildings)
    grid_xs, grid_ys, grid_zs = set(), set(), set()
    if stype in ("building", "warehouse", "pipe_rack", "shed", "portal_frame"):
        bays_x = int(params.get("bays_x", 4))
        bays_y = int(params.get("bays_y", 3))
        sx = float(params.get("spacing_x", 6000))
        sy = float(params.get("spacing_y", 6000))
        sh = float(params.get("story_height", 3500))
        stories = int(params.get("stories", 2))
        grid_xs = {round(i * sx, 1) for i in range(bays_x + 1)}
        grid_ys = {round(i * sy, 1) for i in range(bays_y + 1)}
        grid_zs = {round(sh * i, 1) for i in range(stories + 1)}

    def snap_axis(val, grid_vals):
        if not grid_vals:
            return val
        return min(grid_vals, key=lambda g: abs(g - val))

    for e in elements:
        sp = dict(e.get("StartPoint", {}))
        ep = dict(e.get("EndPoint", {}))
        etype = e.get("Type", "").upper()

        out = False
        for pt in [sp, ep]:
            if (pt.get("X", 0) < footprint["min_x"] - tol or
                pt.get("X", 0) > footprint["max_x"] + tol or
                pt.get("Y", 0) < footprint["min_y"] - tol or
                pt.get("Y", 0) > footprint["max_y"] + tol or
                pt.get("Z", 0) < footprint["min_z"] - tol or
                pt.get("Z", 0) > footprint["max_z"] + tol):
                out = True
                break

        if out:
            rejected.append(e)
            reject_reasons["outside_footprint"] += 1
            continue

        if etype == "SECONDARY" and stype in (
            "building", "warehouse", "pipe_rack", "shed", "portal_frame",
            "office_building", "residential", "stadium", "substation", "parking",
        ):
            sx, sy, sz = sp.get("X", 0), sp.get("Y", 0), sp.get("Z", 0)
            ex, ey, ez = ep.get("X", 0), ep.get("Y", 0), ep.get("Z", 0)
            on_same_x = abs(sx - ex) < tol
            on_same_y = abs(sy - ey) < tol
            if not on_same_x and not on_same_y:
                rejected.append(e)
                reject_reasons["brace_cross_face"] += 1
                continue
            # Snap to grid — keeps braces on structural grid lines
            if on_same_y:
                sp["X"] = snap_axis(sx, grid_xs)
                ep["X"] = snap_axis(ex, grid_xs)
                sp["Y"] = snap_axis(sy, grid_ys)
                ep["Y"] = sp["Y"]
            else:
                sp["Y"] = snap_axis(sy, grid_ys)
                ep["Y"] = snap_axis(ey, grid_ys)
                sp["X"] = snap_axis(sx, grid_xs)
                ep["X"] = sp["X"]
            sp["Z"] = snap_axis(sz, grid_zs)
            ep["Z"] = snap_axis(ez, grid_zs)

        kept.append({**e, "StartPoint": sp, "EndPoint": ep})
...
def place_structure(
    ...
    # 3b. Snap footprint inside Tekla grid (fixes towers at negative XY)
    placed = align_footprint_to_grid(placed, grid_ctx, params, strategy)

    # 4. Deduplicate against existing model (skip for full Create on empty overlap intent)
    if skip_existing_dedup or not existing_members:
        n_removed = 0
    else:
