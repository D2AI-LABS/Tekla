...
from grid_engine           import generate_grid, STRUCTURE_TYPES
from structure_detector    import detect_structure, placement_for_type, is_create_command, extract_primary_command, is_modify_command
from placement_engine      import place_structure, detect_active_grid, validate_structure_geometry
from completion_engine     import complete_model
from topology_engine       import analyse_topology, infer_structure_type
from coordinate_solver     import solve_coordinates, GridConstraints
from universal_rule_engine import validate_structure, get_generation_checklist, list_structure_types
from agent_engine import run_agent_command, infer_agent_placement, AGENT_COMMANDS, cleanup_model
from structure_editor import execute_edit, plan_edit
from structure_planner import plan_structure, verify_components, STRUCTURE_TAXONOMY
...
def write_tekla(payload):
    with open(ELEMENTS_JSON, "w", encoding="utf-8") as f: json.dump(payload, f, indent=2)
...
async def upload_model_json(body: list):
    """Tekla plugin posts extracted model as JSON array."""
    if not isinstance(body, list):
        raise HTTPException(400, "JSON must be a list of members")
    save_json(body)
    st = _tekla_status()
    return {"status": "ok", "members": len(body), "in_sync": True, **st}
...
def bim_generate(req: BIMGenerateRequest):
    ...
    if req.save_to_model and not req.send_to_tekla:
        ...
        # Tekla plugin extracts real model → output.json; avoid phantom dashboard counts
        pass
...
def bim_complete(req: BIMCompleteRequest):
    ...
    if "tower" in itype:
        return {
            "status": "info",
            "message": "Tower models are generated complete via Create command. Use Build Agent: Complete missing members for gaps only.",
            "added": 0,
            "total_in_model": len(existing),
            "topology": {"inferred_type": itype, "completeness_pct": analyse_topology(existing).get("completeness_pct")},
        }

    topo = analyse_topology(existing)
    completion = complete_model(existing, {
        "roof_type": req.roof_type,
        "use_mirror": req.use_mirror,
        "mirror_axis": req.mirror_axis,
    })

    # Merge topology pattern gaps + completion engine gaps (deduplicated)
    pattern_missing = topo.get("missing_by_pattern", [])
    engine_missing  = completion.get("missing") or []
    seen_keys = set()
    all_missing = []
    for e in pattern_missing + engine_missing:
    ...
    for i, m in enumerate(new_members):
        m["Id"]=max_id+i+1; m["Guid"]=str(uuid.uuid4())
    existing.extend(new_members); save_json(existing)

    if req.send_to_tekla:
        write_tekla({"StructureType":"completion",
